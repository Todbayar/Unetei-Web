...
function fetchRecursiveCategoriesDebug($category, $conn, $isInit){
	$categories = [];
	...
	while($row = mysqli_fetch_array($result)){
		...
		$categories = array_merge($categories, fetchRecursiveCategoriesDebug("'c".($tableID+1)."_".$row["id"]."'", $conn, false));
	}
