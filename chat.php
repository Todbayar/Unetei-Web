		...
		if(this.responseText != ""){
			...
			responseChatFetch.forEach(function(chatRow){
				if(parseInt(chatRow.type) == 1){
					...
					chat_list_item(chatRow.id, chatRow.sender, chatRow.body, chatRow.datetime, chatRow.isEdit, chatRow.note, chatRow.isBoost);
				}
	...
	if(sender.id == <?php echo $_COOKIE["userID"]; ?>){
	   ...
	   if(body.isActive == 3){
		...
		if(body.isBoosted){
	   	...
	   	htmlItem += "<a onClick=\"pagenavigation('detail/"+body.id+"','slash')\">"+body.title+" (#"+body.id+")</a>";
		...
		if(isEdit) htmlItem += chat_list_item_action_show(chatID, body.isActive, body.id, true, isBoost);
		...
		htmlItem += "<div style=\"font-size:12px; color:gray; margin-top:2px\">Төлөх:"+(body.pay==0?"Үнэгүй":(body.pay+"₮"+payment))+"</div>";
		htmlItem += paymentHistory+"</div><div class=\"datetime\">"+datetime+"</div>";
		if(isEdit) htmlItem += chat_list_item_action_show(chatID, body.isActive, body.id, false, isBoost);
		htmlItem += "</div></div>";
		$(".chat .right .top").append(htmlItem);
	}
}
...
function chat_list_item_action_show(chatID, isactive, id, isMe = false, isBoost = false){
	...
	else if(isactive==4 && isBoost){
		if(!isMe){
			return "<div id=\"1"+chatID+"\" class=\"action\"><div onClick=\"chat_action("+chatID+",2,1,"+id+")\" class=\"button_yellow\" style=\"float: left\"><i class=\"fa-solid fa-bullhorn\"></i></div></div>";
		}
		else {
			return "<div id=\"1"+chatID+"\" class=\"action\" style=\"display:flex; justify-content: flex-end\"><div onClick=\"chat_action("+chatID+",2,1,"+id+")\" class=\"button_yellow\" style=\"float: left\"><i class=\"fa-solid fa-bullhorn\"></i></div></div>";
		}
	}
	else {
		return "";
...
function chat_action(chatID, action, type, id){
	//chatID chat row id
	//action 0=accept, 1=dismiss, 2=accept boost
...
<div class="chat">
	<div class="left">
		<?php
//		$queryFetchSender = "SELECT c.*, (SELECT name FROM user WHERE id=c.fromID) AS sender_name, (SELECT image FROM user WHERE id=c.fromID) AS sender_image, (SELECT role FROM user WHERE id=c.fromID) AS sender_role, (SELECT phone FROM user WHERE id=c.fromID) AS sender_phone, (SELECT name FROM user WHERE id=c.toID) AS receiver_name, (SELECT image FROM user WHERE id=c.toID) AS receiver_image, (SELECT role FROM user WHERE id=c.toID) AS receiver_role, (SELECT phone FROM user WHERE id=c.toID) AS receiver_phone FROM chat c JOIN (SELECT CASE WHEN fromID=".$_COOKIE["userID"]." THEN toID ELSE fromID END AS other, MAX(datetime) AS latest FROM chat WHERE fromID=".$_COOKIE["userID"]." OR toID=".$_COOKIE["userID"]." GROUP BY other) m ON (c.fromID=".$_COOKIE["userID"]." AND c.toID=m.other OR c.toID=".$_COOKIE["userID"]." AND c.fromID=m.other) AND c.datetime = m.latest ORDER BY c.datetime DESC";
		
		$queryFetchSender = "SELECT ANY_VALUE(c.id) AS id, ANY_VALUE(c.fromID) AS fromID, ANY_VALUE(c.toID) AS toID, ANY_VALUE((SELECT name FROM user WHERE id=c.fromID)) AS sender_name, ANY_VALUE((SELECT image FROM user WHERE id=c.fromID)) AS sender_image, ANY_VALUE((SELECT role FROM user WHERE id=c.fromID)) AS sender_role, ANY_VALUE((SELECT phone FROM user WHERE id=c.fromID)) AS sender_phone, ANY_VALUE((SELECT name FROM user WHERE id=c.toID)) AS receiver_name, ANY_VALUE((SELECT image FROM user WHERE id=c.toID)) AS receiver_image, ANY_VALUE((SELECT role FROM user WHERE id=c.toID)) AS receiver_role, ANY_VALUE((SELECT phone FROM user WHERE id=c.toID)) AS receiver_phone FROM chat c JOIN (SELECT CASE WHEN fromID=".$_COOKIE["userID"]." THEN toID ELSE fromID END AS other, MAX(datetime) AS latest FROM chat WHERE fromID=".$_COOKIE["userID"]." OR toID=".$_COOKIE["userID"]." GROUP BY other) m ON (c.fromID=".$_COOKIE["userID"]." AND c.toID=m.other OR c.toID=".$_COOKIE["userID"]." AND c.fromID=m.other) AND c.datetime = m.latest GROUP BY m.other ORDER BY m.latest DESC";
